...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        boolean nIsEven = n.divide(two).isZero();
        n.multiply(two);
        if (!nIsEven) {
            n.increment();
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        // as your doing expo, take mod right away
        // n is the same as w on the slides (the base num)
        // p is one less than m
        // free lunch
        ...
        if (p.isZero()) {
            n.transferFrom(one);
        } else if (isEven(p) && (p.compareTo(one) > 0)) {
            NaturalNumber remainder = new NaturalNumber2(p);
            n.power(2);
            remainder.divide(two);
            powerMod(n, remainder, m);

        } else if (!isEven(p) && (p.compareTo(one) > 0)) {
            NaturalNumber remainder = new NaturalNumber2(p);
            NaturalNumber staticN = new NaturalNumber2();
            staticN.copyFrom(n);
            n.power(2);
            remainder.divide(two);
            powerMod(n, remainder, m);
        ...
        NaturalNumber2 modM = new NaturalNumber2(n.divide(m));
        n.copyFrom(modM);
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            /*
             * odd n >= 5: check random witnesses that n is composite.
             */
            while (isPrime && count < 50) {
                NaturalNumber randNum = randomNumber(n);
                n.subtract(one);

                if (randNum.compareTo(one) > 0 && randNum.compareTo(n) < 0) {
                    n.add(one);
                    isPrime = !isWitnessToCompositeness(randNum, n);
                    count++;
                } else {
                    n.add(one);
                }
